        ...
        bonusProvider.findTypeWithin(itemType.id, (new Date()), function(error, bonuses) {
          ...
          itemProvider.save({
            ...
            if (items[0]) {
              ...
              _calculateBonuses(req, items[0], req.session.user.login, req.session.user.id, function(error, errorMessage) {
                if (error) { return next(error); }
...
function _calculateBonuses(req, item, login, userId, callback) {
  // find the team
  teamProvider.findById(item.teamId, function(error, team) {
